...
export const QuickActions = () => {
    ...
    const items = [
        { label: 'Aulas', icon: Clock, link: '/cronograma', color: 'text-brand-600', bg: 'bg-brand-50', disabled: false },
            ...
            <div className="grid grid-cols-4 md:grid-cols-5 gap-3">
                {items.map((item: any, idx) => (
                    ...
                    >
                        ...
                        <span className="text-[11px] md:text-sm text-brand-600">{item.label}</span>
                    </Link>
                ))}
            </div>
        </div>
